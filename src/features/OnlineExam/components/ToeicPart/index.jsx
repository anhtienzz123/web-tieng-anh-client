...
function ToeicPart(props) {
	...
	const { questions, examSelected, scrollId, isSubmit } = useSelector(
		(state) => state.exam
	);
	const {
		part1,
	...
	function handleNextClick() {
		...
		if (
			(examSelected === 3 && subPartSelected === part3MaxPage) ||
		...
		switch (selected) {
			case 1:
				return (
					<Part1
						data={part1}
						longAudio={part1Audio}
						onAnswerSheetClick={handleSelected}
					/>
				);
			case 2:
				return (
	...
	return (
		<div id="top" className="toeic_part">
			<div className="toeic_part_content">{renderSwitch(examSelected)}</div>
